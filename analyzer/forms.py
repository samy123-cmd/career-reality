...
class SalaryRealityEngineForm(AccessibleForm):
    ...
    city = forms.ChoiceField(label="City", choices=CITY_CHOICES, widget=forms.Select(attrs=_AZ_SELECT))
    industry = forms.ChoiceField(label="Industry", choices=INDUSTRY_CHOICES, required=False, widget=forms.Select(attrs=_AZ_SELECT))
    company_type = forms.ChoiceField(
...
class OfferAnalyzerForm(AccessibleForm):
    ...
    current_company = forms.CharField(max_length=150, required=False, label="Current company (optional)", widget=forms.TextInput(attrs=_AZ_INPUT))
    current_ctc = forms.IntegerField(required=False, label="Current CTC (LPA)", widget=forms.NumberInput(attrs=_AZ_NUMBER))

    offer_a_company = forms.CharField(max_length=150, label="Offer A — Company", widget=forms.TextInput(attrs=_AZ_INPUT))
    offer_a_role = forms.CharField(max_length=100, required=False, label="Offer A — Role", widget=forms.TextInput(attrs=_AZ_INPUT))
    offer_a_ctc = forms.IntegerField(label="Offer A — CTC (LPA)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_city = forms.ChoiceField(choices=CITY_CHOICES, required=False, widget=forms.Select(attrs=_AZ_SELECT))
    offer_a_fixed_pct = forms.IntegerField(initial=70, min_value=40, max_value=100, label="Fixed %", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_variable_pct = forms.IntegerField(initial=10, min_value=0, max_value=50, label="Variable %", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_esop = forms.IntegerField(required=False, initial=0, label="ESOP value (₹L total)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_joining_bonus = forms.IntegerField(required=False, initial=0, label="Joining bonus (₹L)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_commute = forms.IntegerField(required=False, label="Commute (minutes)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_wlb = forms.IntegerField(required=False, min_value=1, max_value=5, label="WLB 1–5", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_a_work_mode = forms.ChoiceField(choices=[("hybrid", "Hybrid"), ("remote", "Remote"), ("office", "Office")], initial="hybrid", widget=forms.Select(attrs=_AZ_SELECT))
    offer_a_growth = forms.IntegerField(required=False, initial=3, min_value=1, max_value=5, label="Growth potential 1–5", widget=forms.NumberInput(attrs=_AZ_NUMBER))

    offer_b_company = forms.CharField(max_length=150, label="Offer B — Company", widget=forms.TextInput(attrs=_AZ_INPUT))
    offer_b_role = forms.CharField(max_length=100, required=False, label="Offer B — Role", widget=forms.TextInput(attrs=_AZ_INPUT))
    offer_b_ctc = forms.IntegerField(label="Offer B — CTC (LPA)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_b_city = forms.ChoiceField(choices=CITY_CHOICES, required=False, widget=forms.Select(attrs=_AZ_SELECT))
    ...
    offer_b_esop = forms.IntegerField(required=False, initial=0, label="ESOP value (₹L total)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_b_joining_bonus = forms.IntegerField(required=False, initial=0, widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_b_commute = forms.IntegerField(required=False, label="Commute (minutes)", widget=forms.NumberInput(attrs=_AZ_NUMBER))
    offer_b_wlb = forms.IntegerField(required=False, min_value=1, max_value=5, widget=forms.NumberInput(attrs=_AZ_NUMBER))
...
class AICareerImpactForm(AccessibleForm):
    ...
    experience_years = forms.FloatField(label="Years of Experience", initial=5, validators=[MinValueValidator(0), MaxValueValidator(50)], widget=forms.NumberInput(attrs=_AZ_NUMBER))
    industry = forms.ChoiceField(label="Industry", choices=INDUSTRY_CHOICES, required=False, widget=forms.Select(attrs=_AZ_SELECT))
    ...
    tech_stack = forms.CharField(required=False, label="Tech stack (comma-separated)", widget=forms.TextInput(attrs=_AZ_INPUT))
